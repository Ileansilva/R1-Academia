...
function st(c){const d=daysUntil(c.due_date), r=Number(settings.reminder_days||5); if(d<0)return"overdue"; if(d<=r)return"soon"; return"active";}
...
function renderStudents(){
  ...
  $("studentsList").innerHTML=list.length?list.map(student=>{
    const status=student.status==="pending"?"pending":st(student), diff=daysUntil(student.due_date);
    ...
    return `<div class="student-row">
      ...
      <div><span class="status ${status}">${status==="pending"?"Pendente":stLabel(status)}</span></div>
...
window.openStudent=async id=>{
  ...
  $("renewalPanel").classList.toggle("hidden", isPending);
...
async function renderHistory(){
  ...
  $("renewalHistory").innerHTML=history.length
    ?history.map(item=>`<div class="history">Renovado em ${fmtDate(item.renewed_at)} • ${fmtDate(item.old_due_date)} → ${fmtDate(item.new_due_date)} • ${prettyMethod(item.payment_method)}</div>`).join("")
...
function renderDueSoon(){
  const list=students.filter(x=>["soon","overdue"].includes(st(x)));
...
function renderRenewed(){
  $("renewedList").innerHTML=renewals.length ? renewals.map(item=>`
    <div class="renewed-row">
      ...
      <div><span class="status active">${prettyMethod(item.payment_method)}</span></div>
      <span></span><span></span>
    </div>
